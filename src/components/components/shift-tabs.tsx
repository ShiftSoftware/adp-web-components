...
export class ShiftTabs {
  ...
  @Prop() changeActiveTab: (newTabindex: number) => void;
        ...
        <div class={cn('border-b relative flex mb-[6px] ps-[12px] gap-[4px]', this.tabClasses)}>
          {this.tabs.map((label, idx) => (
            <button
              ...
              class={cn(
                'px-[16px] after:w-full py-[6px] after:h-[1px] after:left-0 after:bottom-0 after:absolute after:bg-white after:opacity-0 after:translate-y-full after:z-10 after:duration-500 after:transition text-[17px] relative transition-colors !border-b-0 duration-500 font-medium text-black bg-gray-50 border border-gray-200 rounded-t-[4px] focus:outline-none',
                {
                  'bg-white after:opacity-100': this.activeTabIndex === idx,
                },
              )}
            >
              {label}
