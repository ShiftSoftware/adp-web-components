...
import { FormInputInterface, LocaleFormKeys } from '~types/forms';
      ...
      <Host>
        <label class={cn('relative w-full pb-[20px] inline-flex flex-col', containerClass)}>
          ...
          <input
            ...
            placeholder={texts[placeholder] || texts[label]}
            class={cn(
              'border mb-[4px] disabled:opacity-75 flex-1 py-[6px] px-[12px] transition-all duration-300 rounded-md outline-none focus:border-slate-600 focus:shadow-[0_0_0_0.2rem_rgba(71,85,105,0.25)] w-full',
          ...
          <div class={cn('absolute bottom-0 -z-10 text-red-500 transition-all duration-300', { '-translate-y-full opacity-0': !isError }, errorClass)}>
            {texts[errorMessage] || this.locale.forms.inputValueIsIncorrect}
          </div>
        </label>
      </Host>
    );
